        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            for (auto i = 0; i < ptsx.size(); ++i) {
              double shift_x = ptsx[i] - px; // Puts x to zero
              double shift_y = ptsy[i] - py; // Puts y to zero
              vehicle_xs[i] = (shift_x * cos(0-psi) - shift_y * sin(0-psi));
              vehicle_ys[i] = (shift_x * sin(0-psi) + shift_y * cos(0-psi));
            }
            ...
            // Show reference line
            msgJson["next_x"] = mpc.future_x_values;
            msgJson["next_y"] = mpc.future_y_values;
